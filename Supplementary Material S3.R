...
### R script to simulate and analyse road survey census data for vertebrate groups not affected by carcass location bias
...
#3. Generate Carcass Persistence Curve for pp Prior Based on Santos, Carvalho, and Mira (2011)
...
#4. Calculate alpha and beta Parameters of the Beta Distributed Prior for pp: This
#section makes a beta distributed prior from a mean value and standar error of pp

#5. Census data simulation and Analysis: This section simulates census data 
...
#6. Execution Loop: This section iterates through the defined scenarios:
...
select_group <- function(group_name) {
  if (group_name == "amphibians") {
    ...
    anual_tendency <- "Supplementary material S5/anual_tendency_mammals_g1.csv"
    ppd_group <- 0.388601036
    pom_group <- c(0.6, 0.4, 0.05)
  } else if (group_name == "mammals_g2") {
    anual_tendency <- "Supplementary material S5/anual_tendency_mammals_g2.csv"
    ppd_group <- 0.506361323
    pom_group <- c(0.8, 0.6, 0.2)
  } else if (group_name == "mammals_g3") {
    anual_tendency <- "Supplementary material S5/anual_tendency_mammals_g3.csv"
    ppd_group <- 0.78250591
    pom_group <- c(0.8, 0.6, 0.2)
  } else {
    stop("Invalid group. Please choose between 'amphibians', 'reptiles_g1', 'mammals_g1', 'mammals_g2', or 'mammals_g3'.")
  }
  
  # Note: pl_group is removed as it does not apply in this model
...
# -----------------------------------------------------------------------------
# 3. Generate Carcass Persistence Curve for pp Prior
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 4. Calculate alpha and beta Parameters of the Beta Distributed Prior for pp
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 5. Census data simulation and Analysis 
# -----------------------------------------------------------------------------
...
run_scenario_simulation <- function(sd_lambda, sd_ppd, scenario_id) {
  ...
  for (sim in 1:nsimulations) {
    ...
    # 2. We initialize N3_itD accumulator here
    ...
    for (j in 1:D) {
      ...
      # NOTE: In this model without location bias, N3 depends directly on N (Total number of roadkills)
